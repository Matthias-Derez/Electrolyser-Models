function [mH2, eff_total] = calc_mH2_PEM(jmin, jmax, Tmin, Tmax, Power, Area, Tstep, figures)
% PEM: Calculating mass rate of produced hydrogen
...
% A = 0.37; % m²
A = Area; %m² voorlopig deze waarde om gelijkaardig iets te hebben als bij PEM
...
if figures
    figure(37)
    h = surf(Trange_celcius, jrange, mH2); %Ook hier weer jrangePEM op x-as en TrangePEM_celcius op y-as terwijl het wel mH2_PEM(indexTrange, indexjrange) is
    ...
    title("Hydrogen production rate for a PEM per cell")
    view(30,40)
    grid on
    ...
    set(h,'LineStyle','none')
    title("Efficiency of a PEM electrolyser")
    view(30,40)
    grid on
    xh = get(gca,'XLabel'); % Handle of the x label
    set(xh, 'Units', 'Normalized')
    pos = get(xh, 'Position');
    set(xh, 'Position',pos.*[0.8,-0.0,1],'Rotation',-15)
    yh = get(gca,'YLabel'); % Handle of the y label
    set(yh, 'Units', 'Normalized')
    pos = get(yh, 'Position');
    set(yh, 'Position',pos.*[1.08,-0.7,1],'Rotation',40)
    print -depsc efficiency_PEM.eps

    counter_temp = length(Trange_celcius):-40:1;
    ...
    title("Efficiency of a PEM electrolyser", FontSize=14)
    hold off
