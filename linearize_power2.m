function [coeffs2, error, relative_error] = linearize_power2(jmin, jmax, Tmin, Tmax, Power, num_segments, electrolyser_type, Area, Tstep,figures)
...
for seg_idxT = 1:num_segments
    for seg_idxJ = 1:num_segments
        ...
        Tvector2 = zeros((segment_ends(seg_idxT)-segment_starts(seg_idxT)+1)*2,1); %iedere keer opnieuw gedefinieerd want lengte laatste segment kan verschillen
        jvector2 = zeros((segment_ends(seg_idxT)-segment_starts(seg_idxT)+1)*2,1);
        Powervector2 = zeros((segment_ends(seg_idxT)-segment_starts(seg_idxT)+1)*2,1);
        ...
        for i = segment_starts(seg_idxT):segment_ends(seg_idxT)
                ...
                Tvector2(i-segment_starts(seg_idxT)+1) = Trange(i); 
                jvector2(i-segment_starts(seg_idxT)+1)= jrange(segment_starts(seg_idxJ));
                Powervector2(i-segment_starts(seg_idxT)+1) = Power(segment_starts(seg_idxJ),i);
                Tvector2(i-segment_starts(seg_idxT)+1+segment_ends(seg_idxT)-segment_starts(seg_idxT)+1) = Trange(i); 
                jvector2(i-segment_starts(seg_idxT)+1+segment_ends(seg_idxT)-segment_starts(seg_idxT)+1)= jrange(segment_ends(seg_idxJ));
                Powervector2(i-segment_starts(seg_idxT)+1+segment_ends(seg_idxT)-segment_starts(seg_idxT)+1) = Power(segment_ends(seg_idxJ),i);
        end
...
if figures
    ...
    title(append("Linearised power curve of an ",electrolyser_type," using ",string(num_segments^2), " segments"), "FontSize",14)
    view(30,40)
    grid on
    ...
    for seg_idxj = 1:num_segments-1
        for i = 1 : length(Trange)
            ...
            Pline(i) = Power(segment_ends(seg_idxj),i)+10;
        end
    ...
    for seg_idxT = 1:num_segments-1  
        for i = 1 : length(Trange)
                ...
                Pline(i) = Power(i,segment_ends(seg_idxT))+10;
         end
...
filename = append('C:\Users\matth\Documents\IW5\Thesis\JULIA\','coeffs2',electrolyser_type,string(num_segments),'.csv');
...
if figures
    ...
    zlabel("(Power - Power_{Lin})/Power [-]",FontSize=10)
    set(h,'LineStyle','none')
    ...
    xlabel("Temperature [°C]", FontSize=10)
    ylabel("Current density [A/m²]",FontSize=10)
    zlabel("(Power - Power_{Lin})[W]",FontSize=10)
    set(h,'LineStyle','none')
